...
class ChromatuneApp(tk.Tk):
    def __init__(self):
        ...
        scan = ttk.LabelFrame(self, text="Wavelength Scan", padding=5)
        scan.pack(fill="x", padx=5, pady=5)

        # Start/Stop/Velocity
        row = ttk.Frame(scan, padding=2); row.pack(fill="x")
        ...
        for text,val in [("Continuous","continuous"), ("Stepped","stepped")]:
            ttk.Radiobutton(row, text=text, variable=self.scan_mode, value=val,
        ...
        ttk.Checkbutton(row, text="Repeat until stopped",
                        variable=self.loop_var).pack(side="left", padx=4)
        ...
        self.curr_lbl = ttk.Label(row, text="––– nm"); self.curr_lbl.pack(side="left", padx=4)
        self.prog = ttk.Progressbar(row, length=200); self.prog.pack(side="left", padx=10)
    ...
    def start_scan(self):
        if not self.laser: return messagebox.showwarning("Warning","Connect first.")
        try:
            s = float(self.start_e.get()); e = float(self.stop_e.get())
            v = float(self.vel_e.get()); mode = self.scan_mode.get()
            step = float(self.step_e.get()) if mode=="stepped" else None
        except ValueError:
            return messagebox.showerror("Error","Invalid scan params.")
        total = abs(e - s); self.prog["value"]=0
